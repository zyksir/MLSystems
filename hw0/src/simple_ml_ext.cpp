								  ...
								  float lr, size_t batch)
{
    ...
    for(int iter = 0; iter < iterations; ++iter) {
        ...
        mat_mul(x, theta, Z1, batch, n, k);
        for (int i = 0; i < batch * k; i++) Z1[i] = exp(Z1[i]); // element-wise exp
        for (int i = 0; i < batch; i++) {
            float sum = 0;
            for (int j = 0; j < k; j++) sum += Z1[i * k + j];
            for (int j = 0; j < k; j++) Z1[i * k + j] /= sum; // row-wise normalization
        }
        for(int i = 0; i < batch; ++i) {
            Z1[i*k+yy[i]] -= 1;
        }
