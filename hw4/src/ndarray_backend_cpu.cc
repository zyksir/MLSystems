...
void _stride_out_setter(const AlignedArray* a, AlignedArray* out, std::vector<int32_t> shape,
             std::vector<int32_t> strides, size_t offset, strided_out_mode mode, scalar_t val=-1) {
  const uint32_t ndim = shape.size();
  std::vector<int32_t> out_dim(ndim, 0);
